...
# 黑胶带路径带状搜索参数。path_corners位于胶带中心附近；首次
# 捕获和彻底失锁时只扫描这条带状区域，不扫描墙面背景。
TAPE_SCAN_HALF_WIDTH = 8
TAPE_SCAN_OFFSET_STEP = 2

# 黑胶带上的激光颜色随帧变化很大，因此使用相对周围胶带的
# 局部亮度增量，而不是固定“红色/近白色”硬门限。
TAPE_CONTRAST_MIN_SUM = 220
TAPE_CONTRAST_MIN_DELTA = 80
TAPE_CONTRAST_RING_RADIUS = 3
...
tape_diag_y = 0
tape_diag_sum = 0
tape_diag_delta = -1000
tape_diag_score = -1000000
...
def evaluate_tape_contrast(
    ...
    global tape_diag_x
    global tape_diag_y
    global tape_diag_sum
    global tape_diag_delta
    global tape_diag_score

    ring = TAPE_CONTRAST_RING_RADIUS
    roi_left = LASER_ROI[0] + ring
    roi_top = LASER_ROI[1] + ring
    roi_right = LASER_ROI[0] + LASER_ROI[2] - 1 - ring
    roi_bottom = LASER_ROI[1] + LASER_ROI[3] - 1 - ring

    if x < roi_left or x > roi_right:
        return None
    ...
    # 先用很低的安全下限过滤绝大多数黑胶带背景，只有亮点才
    # 读取周围8个像素，降低K210上的循环开销。
    if brightness < TAPE_CONTRAST_MIN_SUM:
        return None
    ...
    for offset_x, offset_y in (
        ...
        ring_sum += near_pixel[0] + near_pixel[1] + near_pixel[2]

    ring_average = ring_sum // 8
    contrast_delta = brightness - ring_average

    # 记录本帧最强局部增量，便于区分真实激光与均匀白纸/反光。
    if contrast_delta > tape_diag_delta:
        tape_diag_x = x
        tape_diag_y = y
        tape_diag_sum = brightness
        tape_diag_delta = contrast_delta

    if contrast_delta < TAPE_CONTRAST_MIN_DELTA:
        return None
    ...
    # 局部增量是主评分；绝对亮度次之。reference只提供轻微偏好，
    # 不能排除位于矩形其他位置的真实激光。
    score = (
        contrast_delta * 8
        + brightness
        - reference_distance * distance_weight
    )

    if score > tape_diag_score:
        tape_diag_score = score
...
def find_black_tape_laser_core(
    ...
    return LocalCoreBlob(best_x, best_y, best_score)


def find_black_tape_laser_path(
    img,
    path_corners,
    target_x,
    target_y
    ...
    for edge_index in range(4):
        ...
        horizontal_edge = abs(delta_x) >= abs(delta_y)

        for step_index in range(steps + 1):
            base_x = start_point[0] + (
                delta_x * step_index
            ) // steps
            ...
            for offset in range(
                -TAPE_SCAN_HALF_WIDTH,
                TAPE_SCAN_HALF_WIDTH + 1,
                TAPE_SCAN_OFFSET_STEP
            ):
                if horizontal_edge:
                    x = base_x
                    y = base_y + offset
                ...
                if score > best_score:
                    best_score = score
                    best_blob = LocalCoreBlob(x, y, score)
...
def find_laser(
    ...
    # 未锁定：扫描path_corners形成的整条黑胶带带状区域。
    # target只影响同质量候选排序，不限制首次激光所在位置。
    if best_blob is None and use_target_seed:
        best_blob = find_black_tape_laser_path(
            img,
            path_corners,
            center_x,
            center_y
    ...
    while True:
        ...
        if debug_frame_count >= PRINT_INTERVAL:
            ...
            if accepted:
                ...
                if use_target_seed:
                    print(
                        "tape diag target=(%d,%d) "
                        "peak=(%d,%d,sum=%d,delta=%d,score=%d)"
                        % (
                            search_center_x,
                            search_center_y,
                            tape_diag_x,
