...
def find_black_tape_laser_core(
    ...
    for y in range(y0, y1 + 1):
        for x in range(x0, x1 + 1):
            ...
            # 根据黑胶带实测核心：
            # rgb约为(222~247, 255, 255)
            if brightness < 650:
                continue

            if r < 170 or g < 160 or b < 120:
                continue
            ...
            for offset_y in range(-1, 2):
                for offset_x in range(-1, 2):
                    ...
                    if nr >= 100 and current_halo > halo_score:
                        halo_score = current_halo

            # 实测激光光晕red_score约为89~131。
            if halo_score < 70:
                continue
...
def run(
    ...
    while True:
        ...
        if not laser_valid:
            search_center_x = initial_target_x
            search_center_y = initial_target_y
