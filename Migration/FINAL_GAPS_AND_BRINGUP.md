...
- I2C0 异步传输使用 DMA_CH2/CH3；短命令组合读取分成写命令与重复START读两个硬件阶段，`TX_DONE`后由后台任务确认Controller FSM空闲并启动读阶段，避免`RD_ON_TXEMPTY`异常残留。
...
- 2026-07-26第三轮现场日志已确认组合事务成功读回`ping=0x66`，但旧配置在最后一个接收字节继续发送ACK，且软件在`RX_DONE`时早于STOP完成释放事务，现场因此停在`SCL=1、SDA=0、IDLE=1、BUSBSY=1`。现已按TI SDK的repeated-start示例将控制器末字节配置改为NACK，并将成功回调延后到`IDLE=1、BUSBSY=0`；此前针对`I2C_ERR_05`、`I2C_ERR_07`和`I2C_ERR_13`的修正继续保留。已通过启用专项测试的TI Clang完整编译，仍需断电重启主控和灰度模块后按测试手册第19节确认`valid=1`、8路数据持续更新且OLED仍能共用总线。
- 2026-07-27现场改用灰度手册方法1后，`0xB0+8字节`可以连续返回正确数据，但偶发NACK后硬件`RD_ON_TXEMPTY`会停在`IDLE=1、BUSBSY=1`且不再产生RX_DONE，随后ping也无法恢复。当前组合事务已改为软件两阶段：写阶段不发STOP，`TX_DONE`后由`BSP_I2C_Task()`确认FSM空闲，再用一次完整MCTR写产生重复START和读阶段；超时恢复只在线路均为高且Controller不忙时复位外设，以遵守`I2C_ERR_05`。启用专项测试的TI Clang全量clean build已通过，仍待目标板持续运行验证。
- 灰度按键校准原先把提示误发到E220使用的`UART_PORT1`，且传感器离线或采样顺序错误时不更新显示，现场会表现为按键无反应。现已改用`DEBUG_UART_PORT`，并为LCD/OLED增加传感器离线和采样不完整页面；已通过TI Clang完整编译，仍需按测试手册第18A节实机确认。
- 共享I2C短暂占用曾使`Drv_GrayMcu_IsOnline()`在样本年龄刚超过100 ms时拒绝按键校准；现改为以最近有效样本为准并将陈旧时限放宽到500 ms。循迹日志也已将`RX8`、`RAW/FILT`、阈值和循迹结果提前到I2C详细诊断之前，避免512字节UART环满时只丢失较长的灰度数据行。
- ICM20948六轴降级路径已经通过TI Clang完整编译，仍需按测试手册第23、24节烧录复验。
